...
import type { ProcessComponentProps, ProcessComponentRef } from "./types";

type CanvasProps = ProcessComponentProps;

export const Canvas = forwardRef<ProcessComponentRef, CanvasProps>(
	function Canvas(props, ref) {
		...
		const process = useMemo(() => {
			return Kernel.instance().scheduler.processes.get(+pid);
		}, [pid]);
		...
		useEffect(() => {
			...
			const { data: offscreen, error } = safe(() =>
